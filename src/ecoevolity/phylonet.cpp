...
#include "phylonet.hpp"

void BasePopulationNetwork::set_root(std::shared_ptr<PopulationNetNode> root) {
    BaseTree::set_root(root);
    this->root_->resize_all();
}

BasePopulationNetwork::BasePopulationNetwork(
...
void BasePopulationNetwork::establish_tree_and_node_heights(
        const PopulationTreeSettings& settings,
        RandomNumberGenerator& rng) {
...
void BasePopulationNetwork::establish_branch_and_mu_parameters(
        const PopulationTreeSettings& settings,
        RandomNumberGenerator& rng) {
...
void BasePopulationNetwork::draw_from_prior(RandomNumberGenerator& rng) {
...
void BasePopulationNetwork::write_state_log_header(std::ostream& out,
...
void BasePopulationNetwork::log_state(std::ostream& out,
...
double BasePopulationNetwork::get_ln_prob_of_drawing_node_state(
                std::shared_ptr<PopulationNetNode> node) const {
    if (this->population_sizes_are_constrained()) {
        return 0.0;
    }
    if (this->population_sizes_are_fixed()) {
...
BasePopulationNetwork::BasePopulationNetwork(
        ...
        bool validate_data) : BaseTree<PopulationNetNode>() {
    const std::vector< std::shared_ptr<PopulationNetNode> >& leaves = root->get_leaves();
...
void BasePopulationNetwork::init_data(
        std::string path, 
        char population_name_delimiter,
        bool population_name_is_prefix,
        bool genotypes_are_diploid,
...
void BasePopulationNetwork::init(
        std::string path, 
        char population_name_delimiter,
        bool population_name_is_prefix,
        bool genotypes_are_diploid,
...
void BasePopulationNetwork::process_and_vet_initialized_data(
        bool strict_on_constant_sites,
        bool strict_on_missing_sites,
        bool strict_on_triallelic_sites) {
    if (this->data_.get_number_of_populations() < 1) {
        throw EcoevolityError("BasePopulationNetwork(); no populations were found");
    }
...
void BasePopulationNetwork::init_tree() {
    if (this->data_.get_number_of_populations() < 3) {
        std::shared_ptr<PopulationNetNode> root = std::make_shared<PopulationNetNode>(this->data_.get_number_of_populations(), 0.0);
        for (unsigned int pop_idx = 0;
                pop_idx < this->data_.get_number_of_populations();
                ++pop_idx) {
            std::shared_ptr<PopulationNetNode> tip = std::make_shared<PopulationNetNode>(
                    pop_idx,
    ...
    std::shared_ptr<PopulationNetNode> ancestor = std::make_shared<PopulationNetNode>(next_index, 0.0);
    ++next_index;
    ancestor->add_child(std::make_shared<PopulationNetNode>(
                0,
    ...
    for (unsigned int pop_idx = 2;
            pop_idx < this->data_.get_number_of_populations();
            ++pop_idx) {
        std::shared_ptr<PopulationNetNode> next_ancestor = std::make_shared<PopulationNetNode>(next_index, 0.0);
        ++next_index;
...
void BasePopulationNetwork::set_tree_to_comb() {
    std::shared_ptr<PopulationNetNode> root = std::make_shared<PopulationNetNode>(
...
void BasePopulationNetwork::set_tree_to_random_bifurcating(
        RandomNumberGenerator & rng) {
    unsigned int next_index = this->data_.get_number_of_populations();
    std::shared_ptr<PopulationNetNode> root = std::make_shared<PopulationNetNode>(
            next_index,
    ...
    std::vector< std::shared_ptr<PopulationNetNode> > existing_nodes;
    ...
    std::shared_ptr<PopulationNetNode> leaf1 = std::make_shared<PopulationNetNode>(
            leaf_idx,
    ...
    leaf_indices.pop_back();
    std::shared_ptr<PopulationNetNode> leaf2 = std::make_shared<PopulationNetNode>(
            leaf_idx,
            this->data_.get_population_label(leaf_idx),
            0.0,
            this->data_.get_max_allele_count(leaf_idx));
    ...
    std::shared_ptr<PopulationNetNode> sib_node;
    std::shared_ptr<PopulationNetNode> parent_node;
    while(leaf_indices.size() > 0) {
        leaf_idx = leaf_indices.back();
        leaf_indices.pop_back();
        std::shared_ptr<PopulationNetNode> leaf = std::make_shared<PopulationNetNode>(
                leaf_idx,
                this->data_.get_population_label(leaf_idx),
                0.0,
                this->data_.get_max_allele_count(leaf_idx));
        ...
        std::shared_ptr<PopulationNetNode> new_node = std::make_shared<PopulationNetNode>(
                next_index,
                new_height);
        ++next_index;
...
void BasePopulationNetwork::set_tree_to_upgma() {
    throw EcoevolityNotImplementedError(
            "BasePopulationNetwork::set_tree_to_upgma is not implemented");
}

void BasePopulationNetwork::set_tree_from_path(const std::string & path) {
...
void BasePopulationNetwork::set_tree_from_string(const std::string & newick_tree_string) {
    bool root_ht_fixed = this->root_height_is_fixed();
    std::shared_ptr<ContinuousProbabilityDistribution> root_ht_prior = this->get_root_node_height_prior();
...
void BasePopulationNetwork::check_if_leaf_labels_match_data() const {
...
void BasePopulationNetwork::update_leaf_label_indices() {
...
void BasePopulationNetwork::update_unique_allele_counts() {
...
void BasePopulationNetwork::calculate_likelihood_correction() {
...
double BasePopulationNetwork::get_likelihood_correction(bool force) {
...
double BasePopulationNetwork::calculate_log_binomial(
...
// bool BasePopulationNetwork::constant_site_counts_were_provided() {
...
void BasePopulationNetwork::fold_patterns() {
...
void BasePopulationNetwork::set_freq_1(double p) {
...
double BasePopulationNetwork::get_freq_1() const {
    return this->freq_1_->get_value();
}
double BasePopulationNetwork::get_freq_0() const {
    return 1.0 - this->get_freq_1();
}
double BasePopulationNetwork::get_u() const {
    ECOEVOLITY_ASSERT(this->get_freq_1() > 0.0);
    return 1.0 / (2.0 * this->get_freq_1());
}
double BasePopulationNetwork::get_v() const {
    // double u = this->get_u();
    // return u / ((2.0 * u) - 1.0);
    ECOEVOLITY_ASSERT(this->get_freq_0() > 0.0);
    return 1.0 / (2.0 * this->get_freq_0());
}
void BasePopulationNetwork::store_freq_1() {
    this->freq_1_->store();
}
void BasePopulationNetwork::restore_freq_1() {
    this->freq_1_->restore();
    this->make_dirty();
}

void BasePopulationNetwork::set_mutation_rate(double m) {
    if (this->mutation_rate_is_fixed()) {
        return;
    }
...
double BasePopulationNetwork::get_mutation_rate() const {
    return this->mutation_rate_->get_value();
}
void BasePopulationNetwork::store_mutation_rate() {
    this->mutation_rate_->store();
}
void BasePopulationNetwork::restore_mutation_rate() {
    this->mutation_rate_->restore();
    this->make_dirty();
}

std::shared_ptr<PositiveRealParameter> BasePopulationNetwork::get_freq_1_parameter() const {
    return this->freq_1_;
}

void BasePopulationNetwork::set_mutation_rate_parameter(std::shared_ptr<PositiveRealParameter> h) {
    this->mutation_rate_ = h;
    this->make_dirty();
}
std::shared_ptr<PositiveRealParameter> BasePopulationNetwork::get_mutation_rate_parameter() const {
    return this->mutation_rate_;
}

void BasePopulationNetwork::set_root_population_size(double size) {
    if (this->root_->population_size_is_fixed()) {
        return;
    }
    this->root_->set_population_size(size);
}
void BasePopulationNetwork::set_all_population_sizes(double size) {
    if (this->population_sizes_are_fixed()) {
        return;
    }
    this->root_->set_all_population_sizes(size);
}
unsigned int BasePopulationNetwork::scale_all_population_sizes(double scale) {
    if (this->population_sizes_are_fixed()) {
        return 0;
    }
...
unsigned int BasePopulationNetwork::scale_root_population_size(double scale) {
    if (this->root_->population_size_is_fixed()) {
        return 0;
    }
...
double BasePopulationNetwork::get_root_population_size() const {
    return this->root_->get_population_size();
}
std::shared_ptr<PositiveRealParameter> BasePopulationNetwork::get_root_population_size_parameter() const {
...
void BasePopulationNetwork::store_derived_class_parameters() {
    // Storing of pop sizes is taken care of by PopulationNetNode
    this->store_freq_1();
    this->store_mutation_rate();
}
void BasePopulationNetwork::store_all_population_sizes() {
    this->root_->store_all_population_sizes();
}

void BasePopulationNetwork::restore_derived_class_parameters() {
    // Restoring of pop sizes is taken care of by PopulationNetNode
    this->restore_freq_1();
    this->restore_mutation_rate();
}
void BasePopulationNetwork::restore_all_population_sizes() {
    this->root_->restore_all_population_sizes();
}

void BasePopulationNetwork::set_population_size_prior(std::shared_ptr<ContinuousProbabilityDistribution> prior) {
    this->population_size_prior_ = prior;
    this->root_->set_all_population_size_priors(prior);
}

void BasePopulationNetwork::set_freq_1_prior(std::shared_ptr<ContinuousProbabilityDistribution> prior) {
    this->freq_1_->set_prior(prior);
    this->make_dirty();
}
void BasePopulationNetwork::set_mutation_rate_prior(std::shared_ptr<ContinuousProbabilityDistribution> prior) {
    this->mutation_rate_->set_prior(prior);
    this->make_dirty();
}

double BasePopulationNetwork::get_derived_class_component_of_log_prior_density() const {
    double d = 0.0;
...
double BasePopulationNetwork::compute_log_prior_density_of_state_frequencies() const {
    return this->freq_1_->relative_prior_ln_pdf();
}
double BasePopulationNetwork::compute_log_prior_density_of_mutation_rate() const {
    return this->mutation_rate_->relative_prior_ln_pdf();
}
double BasePopulationNetwork::compute_log_prior_density_of_population_sizes() const {
    return this->root_->calculate_ln_relative_population_size_prior_density();
}

// void BasePopulationNetwork::provide_number_of_constant_sites(
...
std::shared_ptr<GeneTreeSimNode> BasePopulationNetwork::simulate_gene_tree(
        const unsigned int pattern_index,
        RandomNumberGenerator& rng,
        const bool use_max_allele_counts) const {
    std::unordered_map<std::shared_ptr<const PopulationNetNode>, std::vector< std::shared_ptr<GeneTreeSimNode> > > branch_lineages;
...
bool BasePopulationNetwork::sample_pattern(
        RandomNumberGenerator& rng,
...
BiallelicData BasePopulationNetwork::simulate_biallelic_data_set(
        RandomNumberGenerator& rng,
...
BiallelicData BasePopulationNetwork::simulate_linked_biallelic_data_set(
        RandomNumberGenerator& rng,
        float singleton_sample_probability,
...
BasePopulationNetwork::simulate_complete_biallelic_data_set(
        RandomNumberGenerator& rng,
...
std::pair<BiallelicData, unsigned int>
BasePopulationNetwork::simulate_data_set_max_one_variable_site_per_locus(
        RandomNumberGenerator& rng,
        unsigned int locus_size,
        float singleton_sample_probability,
        bool validate) const {
...
BasePopulationNetwork::simulate_biallelic_site(
...
std::pair<std::vector<unsigned int>, std::vector<unsigned int> >
BasePopulationNetwork::simulate_biallelic_site_sans_missing(
        std::shared_ptr<GeneTreeSimNode> gene_tree,
...
double BasePopulationNetwork::compute_log_likelihood(
    ...
    if (std::isnan(log_likelihood)) {
        throw EcoevolityError("BasePopulationNetwork::compute_log_likelihood resulted in a NAN likelihood");
    }
