...
void write_sumcoevolity_splash(std::ostream& out) {
    ...
    out << string_util::banner('=') << "\n" 
        << string_util::center("Sumcoevolity") << "\n"
        << string_util::center("Summarizing evolutionary coevality") << "\n\n"
...
void check_sumcoevolity_output_path(const std::string& path) {
    if (path::exists(path)) {
        ...
        message << "ERROR: The sumcoevolity output file \'"
                << path
                << "\' already exists!\n"
                << "Please use a different output prefix\n";
