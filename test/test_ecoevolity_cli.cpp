...
TEST_CASE("Testing missing population sites", "[EcoevolityCLI]") {
    ...
    SECTION("Testing missing population sites") {
        ...
        // We now allow populations to have no data for some or all characters,
        // so this should no longer throw and error
        ret = ecoevolity_main<CollectionSettings, ComparisonPopulationTreeCollection>(argc, argv);
        REQUIRE(ret == 0);
...
TEST_CASE("Testing relaxed fully missing sites setting", "[EcoevolityCLI]") {

    SECTION("Testing fully missing sites relaxed") {
        double height_shape = 10.0;
        double height_scale = 0.001;
        double size1_shape = 10.0;
        double size1_scale = 0.0001;
        ...
        // One comparison has a site that is missing for all populations, but
        // we are asking ecoevolity to remove it and not throw an error
        ret = ecoevolity_main<CollectionSettings, ComparisonPopulationTreeCollection>(argc, argv);
        REQUIRE(ret == 0);
...
TEST_CASE("Testing fully missing sites error", "[EcoevolityCLI]") {
    ...
    SECTION("Testing fully missing sites error") {
        ...
        // One comparison has a site with no data for any of the populations,
...
TEST_CASE("Testing relaxed triallelic sites setting", "[EcoevolityCLI]") {

    SECTION("Testing triallelic sites relaxed") {
        double height_shape = 10.0;
        double height_scale = 0.001;
        double size1_shape = 10.0;
        double size1_scale = 0.0001;
        ...
        char arg2[] = "283402";
        char arg3[] = "--ignore-data";
        char arg4[] = "--relax-triallelic-sites";
        char * cfg_path = new char[test_path.size() + 1];
        std::copy(test_path.begin(), test_path.end(), cfg_path);
        cfg_path[test_path.size()] = '\0';
        char * argv[] = {
...
TEST_CASE("Testing triallelic sites error", "[EcoevolityCLI]") {
