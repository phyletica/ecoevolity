...
TEST_CASE("Testing SplitLumpNodesRevJumpSampler operator with auto-tuning",
        "[GeneralTreeOperatorSchedule]") {
...
TEST_CASE("Testing SubtreePruneRegraftRevJumpSampler operator",
        "[GeneralTreeOperatorSchedule]") {
    SECTION("Testing SubtreePruneRegraftRevJumpSampler") {
        std::string cfg_path = "data/dummy.yml";
        ...
        cfg_stream << "    sample_frequency: 10\n";
        cfg_stream << "    operators:\n";
        cfg_stream << "        SubtreePruneRegraftRevJumpSampler:\n";
        cfg_stream << "            weight: [2, 3, 5]\n";
        cfg_stream << "            tuning_parameter: [1.0, 0.3, 1.9]\n";
        cfg_stream << "            auto_optimize: [true, false, false]\n";
        cfg_stream << "            auto_optimize_delay: [100, 50, 20]\n";

        PopulationTreeSettings settings = PopulationTreeSettings(cfg_stream, cfg_path);

        auto rj_op = settings.operator_settings->tunable_operators["SubtreePruneRegraftRevJumpSampler"];
        for (unsigned int i = 0; i < rj_op.get_number_of_operators(); ++i) {
            std::cout << "weight " << i << ": " << rj_op.get_weight(i) << "\n";
            std::cout << "tuning param " << i << ": " << rj_op.get_tuning_parameter(i) << "\n";
            std::cout << "auto opt " << i << ": " << rj_op.auto_optimizing(i) << "\n";
        ...
        ops = op_schedule.get_spr_rj_operators();
        REQUIRE(ops.size() == 3);
        ...
        for (auto oper : ops) {
            ...
            if (oper->get_weight() == 2.0) {
                ...
                REQUIRE(oper->get_coercable_parameter_value() == 0.3);
                REQUIRE(oper->auto_optimizing() == false);
                REQUIRE(oper->get_auto_optimize_delay() == 50);
            }
            else if (oper->get_weight() == 5.0) {
                REQUIRE(oper->get_coercable_parameter_value() == 1.9);
                REQUIRE(oper->auto_optimizing() == false);
                REQUIRE(oper->get_auto_optimize_delay() == 20);
            }
            else {
...
TEST_CASE("Testing SubtreePruneRegraftRevJumpSampler operator with some defaults",
        "[GeneralTreeOperatorSchedule]") {
    SECTION("Testing SubtreePruneRegraftRevJumpSampler") {
        std::string cfg_path = "data/dummy.yml";
        ...
        cfg_stream << "    operators:\n";
        cfg_stream << "        SubtreePruneRegraftRevJumpSampler:\n";
        cfg_stream << "            tuning_parameter: [1.0, 2.0, 4.0]\n";
        ...
        std::set<double> expected_tuning_parameters;
        expected_tuning_parameters.insert(1.0);
        expected_tuning_parameters.insert(2.0);
        expected_tuning_parameters.insert(4.0);
        ...
        for (auto oper : ops) {
            REQUIRE(oper->helper_ops.size() > 0);
            tuning_parameters.insert(oper->get_coercable_parameter_value());
            if (oper->get_coercable_parameter_value() == 1.0) {
                REQUIRE(oper->get_weight() == 3.0);
                REQUIRE(oper->auto_optimizing() == false);
                REQUIRE(oper->get_auto_optimize_delay() == 50);
            }
            else if (oper->get_coercable_parameter_value() == 2.0) {
                REQUIRE(oper->get_weight() == 3.0);
                REQUIRE(oper->auto_optimizing() == false);
                REQUIRE(oper->get_auto_optimize_delay() == 50);
            }
            else if (oper->get_coercable_parameter_value() == 4.0) {
                REQUIRE(oper->get_weight() == 3.0);
                REQUIRE(oper->auto_optimizing() == false);
                REQUIRE(oper->get_auto_optimize_delay() == 50);
            }
...
TEST_CASE("Testing SubtreePruneRegraftRevJumpSampler operator with zero weight",
        "[GeneralTreeOperatorSchedule]") {
    SECTION("Testing SubtreePruneRegraftRevJumpSampler with zero weight") {
        std::string cfg_path = "data/dummy.yml";
...
TEST_CASE("Testing SubtreePruneRegraftRevJumpSampler operator turn off",
        "[GeneralTreeOperatorSchedule]") {
    SECTION("Testing SubtreePruneRegraftRevJumpSampler turn off") {
        std::string cfg_path = "data/dummy.yml";
